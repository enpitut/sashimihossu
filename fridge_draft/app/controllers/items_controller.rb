class ItemsController < ApplicationController
  ...
  #=================================================================================================================================================
  # レシピ提案
  # 検索したいアイテムを配列で渡してやることでcookpadをOR検索してスクレイピング，
  # 結果を
  def recommend_recipe(queries)
    ##############################
    # Randomクラスの定義
    ##############################
    random = Random.new
    ...
    # レシピ管理クラス
    ##############################
    Recipe = Struct.new(:id, :title, :amount, :url, :date)
    ...
    Anemone.crawl(uri.to_s, opts) do |anemone|
      ...
      anemone.on_pages_like(%r{http://cookpad.com/recipe/[0-9]+}) do |page|
        ...
        page.doc.xpath("//div[@id='main']").each do |node|
          ...
          ##############################
          if (title != "")
            recipe_list << Recipe.new(id_num, title, amount, url, date)
    ...
    File.open("./tmp/recipe.txt", "w") do |file|
      for recipe in recipe_list do
        ...
        file.puts "--------------------------------------------------\n\n"
        for ingredient in ingredient_list do
